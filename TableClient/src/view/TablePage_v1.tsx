//! copy of old component - bug in refresh!
...
function TablePage() {
  ...
  if (!tableId) {
    console.log("no tableId in params");
  ...
  if (!table || table === undefined) {
    console.log(
      `Table with ID ${tableId} not found. Waiting for tables to load...`
    );
    return <div>Loading table information...</div>;
  }
  ...
  console.log("After calling generateCellsForPlot displayArr is:",displayArr)
  ...
  useEffect(() => {
    ...
    // Step 2: Check if data has been fetched already during this session
    const isRefreshed = sessionStorage.getItem("isRefreshed");

    // Step 3: If data was already fetched, skip fetching again
